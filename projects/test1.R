...
# 2 - Use a Poisson distribution with means = 5, 6, 7 to 
# generate 3 gene expression datasets representing treated
treated5 <- rpois(num_genes, lambda = 5)
treated6 <- rpois(num_genes, lambda = 6)
treated7 <- rpois(num_genes, lambda = 7)

# 3 - Repeat the above with means = 1, 2, 1 to generate 3 
# gene expression datasets representing untreated.
untreat1 <- rpois(num_genes, lambda = 1)
untreat2 <- rpois(num_genes, lambda = 2)
untreat3 <- rpois(num_genes, lambda = 1)

# 4 - Generate a data frame whose rows are genes (20 total) 
# and whose columns are samples 
# (ca1, ca2, ca3 representing cases and co1, co2, co3, representing controls)
data <- data.frame(
